...
function setup() {
  ...
  invisibleGround = createSprite(570,550,200,30);
...
function draw() 
{
  ...
  if(lander.collide(ground)==true)
  {
    ...
    stroke("black");
    strokeWeight(3);
    if (lander.isTouching(invisibleGround)){
     // vx = 0;
     // vy = 0;
     // g=0;
     // lander.changeAnimation('landing');
    ...
    stroke("black");
    strokeWeight(3);
       text ("Lander landed successfully!!",400,350);
      }
      text("<--- Land the lander here",650,550);
        drawSprites();
}
